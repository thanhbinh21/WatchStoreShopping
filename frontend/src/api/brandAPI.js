    ...
    try {
        ...
        if (typeof data === 'string') {
            try {
                ...
                console.error("brandAPI: String preview (first 1000 chars):", data.substring(0, 1000));
                console.error("brandAPI: String preview (last 500 chars):", data.substring(data.length - 500));
                ...
                if (arrayStartIndex !== -1) {
                    ...
                    if (arrayEndIndex !== -1) {
                        const arrayString = data.substring(arrayStartIndex, arrayEndIndex + 1);
                        try {
                            parsedData = JSON.parse(arrayString);
                            if (Array.isArray(parsedData)) {
                                console.log("brandAPI: Successfully extracted array from string");
                                data = parsedData;
                            }
                        } catch (e) {
                ...
                if (!parsedData) {
                    ...
                    if (arrayMatch && arrayMatch[0]) {
                        try {
                            ...
                            if (Array.isArray(parsedData)) {
                                console.log("brandAPI: Successfully extracted array using regex");
                                data = parsedData;
                            }
                        } catch (e) {
                            console.error("brandAPI: Failed to parse regex-extracted array:", e);
                        }
                    }
                }
                ...
                if (!parsedData || !Array.isArray(data)) {
                    console.warn("brandAPI: Could not parse string, trying manual extraction");
                    ...
                    if (brandMatches && brandMatches.length > 0) {
                        ...
                        if (extractedBrands.length > 0) {
                            console.log("brandAPI: Extracted", extractedBrands.length, "brands manually");
        ...
        if (Array.isArray(data)) {
            ...
            const cleanedBrands = data.map(brand => {
            ...
            console.log("brandAPI: Returning", cleanedBrands.length, "cleaned brands");
